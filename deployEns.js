...
async function deployContract(abi, bytecode, wait,  args=[]){
	...
	const wlt = new ethers.Wallet("0x0dbc2427bcc0c03b4d7568f0ac135b543633c237d06837e8ff6dabc8ca69b3ae", provider)
...
async function deployTemp(){
    ...
    const ownerAddress = "0x9A0d73b6D2D54fdC66AdD5b73495DB059Ce24C5c"
    ...
    // Now we are the owner of namehash.hash("web2") but not of keccak256("eth")
    await tx.wait()
...
async function main(){
    ...
    const newUser = await registrar.register(ethers.keccak256(Buffer.from("tes")), "0x9A0d73b6D2D54fdC66AdD5b73495DB059Ce24C5c", ttl)
    await newUser.wait()
}
main()
