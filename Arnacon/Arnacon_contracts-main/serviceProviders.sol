...
contract serviceProviders is ERC721, ERC721URIStorage, ERC721Burnable, Ownable{
    ...
    function safeMint(
        ...
        bytes32 namehash = computeNamehash(_serviceProviderName);   // get the node of the ens
        address addressOfEns = EnsContract.owner(namehash); // find address in ENS contract
        require(
            addressOfEns == _addressOfEns, 
    ...
    function computeNamehash(string memory _name) public pure returns (bytes32 namehash) {
        ...
        namehash = keccak256(
        abi.encodePacked(namehash, keccak256(abi.encodePacked('cellact')))
        );
        namehash = keccak256(
